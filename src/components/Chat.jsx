    ...
    <div className="nav-item absolute right-5 md:right-52 top-16 bg-white dark:bg-[#42464D] p-8 rounded-lg w-96">
      <div className="flex justify-between items-center">
        <div className="flex gap-3">
          ...
          <button type="button" className="text-xs rounded p-1 px-2 bg-orange text-gray-400">
            4 New
          </button>
        </div>
        <Button
          ...
          {chatData?.map((item,index) => (
            <div key={index} className="flex items-center gap-5 border-b-1 border-color p-3 leading-8 cursor-pointer">
              <div className="relative">
                ...
                alt={item.message}
                 />
                <span style={{background:"orange"}} className="absolute inline-flex rounded-full h-2 w-2 right-0 top-1">
                </span>
              </div>
              <div>
